          ...
          try {
            ...
            if (pathname === '/cms/images' && method === 'GET') {
              try {
                ...
                return sendJson(
                  res,
                  files.filter((f) => /\.(png|jpe?g|gif|webp|svg|avif)$/i.test(f))
                );
              } catch {
                return sendJson(res, []);
              }
            ...
            if (pathname === '/cms/images' && method === 'POST') {
              ...
              if (!file) return sendJson(res, { error: 'No file part found' }, 400);
            ...
            if (imgDeleteMatch && method === 'DELETE') {
              const filename = path.basename(decodeURIComponent(imgDeleteMatch[1]));
