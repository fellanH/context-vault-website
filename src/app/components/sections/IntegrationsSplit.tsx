            ...
            {integrations.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-6">
                {integrations.map((item) => (
                  <div
                    key={item.name}
                    className="flex items-center gap-2 rounded-full border border-border px-3 py-2 text-xs text-muted-foreground overflow-hidden"
                  >
                    {item.logo && (
                      <img
                        src={`https://cdn.simpleicons.org/${item.logo}/717171`}
                        width={14}
                        height={14}
                        alt=""
